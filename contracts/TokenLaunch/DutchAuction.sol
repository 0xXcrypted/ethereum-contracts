...
// Dutch Auction for Decipher Session
...
contract DutchAuction is ReentrancyGuard, Ownable  {
    ...
    function initAuction(
        ...
        require(ERC20(_token).decimals() == 18, "DutchAuction: Token does not have 18 decimals");
        if (_paymentCurrency != ETH_ADDRESS) {
            require(ERC20(_paymentCurrency).decimals() > 0, "DutchAuction: Payment currency is not ERC20");
        }
   ...
   /**
     ...
     */
    function setAuctionPrice(uint256 _startPrice, uint256 _minimumPrice) external onlyOwner{
        // require(hasAdminRole(msg.sender));
        require(_startPrice > _minimumPrice, "DutchAuction: start price must be higher than minimum price");
        require(_minimumPrice > 0, "DutchAuction: minimum price must be greater than 0"); 
        require(marketStatus.commitmentsTotal == 0, "DutchAuction: auction cannot have already started");
