...
contract UniswapExchange is ERC20 {
  ...
  function setup(address token_addr) public {
    ...
    name = 0x556e697377617020563100000000000000000000000000000000000000000000; // Uniswap V1
    symbol = 0x554e492d56310000000000000000000000000000000000000000000000000000; // UNI-V1
    decimals = 18;
  }
 ...
 /**
  ...
  function getInputPrice(uint256 input_amount, uint256 input_reserve, uint256 output_reserve) public view returns (uint256) {
    require(input_reserve > 0 && output_reserve > 0, "INVALID_VALUE_getInputPrice");
 ...
 /**
    ...
    private returns (uint256) 
  {
    ...
    require(token.transferFrom(buyer, address(this), tokens_sold), '2');
  ...
  function removeLiquidity(uint256 amount, uint256 min_eth, uint256 min_tokens, uint256 deadline) public returns (uint256, uint256) {
    require(amount > 0 && deadline > block.timestamp && min_eth > 0 && min_tokens > 0, 'UniswapExchange#removeLiquidity: INVALID_ARGUMENT');
    uint256 total_liquidity = _totalSupply;
